...
function KiemDinhCapMoi({ isLoading }) {
  ...
  const getDataFilter = async () => {
    ...
    const apiResponse = await getAllGiayToThamDinh(page, limit, queryStr);
    if (apiResponse) {
        ...
        return (
          <div className="action-dv">
            <Button
              ...
              style={{ border: 0 }}
              onClick={() => {
                history.push(URL.CHI_TIET_CAP_MOI_ID.format(value._id));
              }}
            ></Button>
          </div>
        );
...
export default connect(mapStateToProps)(KiemDinhCapMoi);
